...
impl EnvUniversal {
    ...
    pub fn format_for_contents(s: &[u8]) -> UvarFormat {
        ...
        for line in iter {
            ...
            return if unsafe { versionbuf.assume_init() } == UVARS_VERSION_3_0 {
                UvarFormat::fish_3_0
            } else {
                UvarFormat::future
            };
        ...
        if success {
            ...
            if unsafe { UVAR_FILE_SET_MTIME_HACK() } {
                ...
                if unsafe { libc::clock_gettime(CLOCK_REALTIME, &mut times[1]) } != 0 {
                    unsafe {
                        libc::futimens(private_fd.fd(), &mut times[0]);
                    }
                }
            }
...
fn full_escape(input: &wstr) -> WString {
    ...
    for c in input.chars() {
        if is_universal_safe_to_encode_directly(c) {
            ...
            sprintf!(=> &mut out, "\\x%.2x", c);
        } else if u32::from(c) < 65536 {
            sprintf!(=> &mut out, "\\u%.4x", c);
        } else {
            sprintf!(=> &mut out, "\\U%.8x", c);
        }
    }
    out
}
