...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < argc || (argc == 0 && i == 0); i++) {
        ...
        maybe_t<rust::Box<HighlightSpecListFFI>> rust_colors;
        if (output_type != output_type_plain_text) {
            highlight_shell(output_wtext, colors, *operation_context_globals());
            rust_colors =
                highlight_shell_ffi(output_wtext, *operation_context_globals(), false, {});
        }
        ...
        switch (output_type) {
            ...
            case output_type_ansi: {
                auto rust_colored = colorize(output_wtext, **rust_colors, env_stack_globals());
                for (uint8_t c : rust_colored) {
